...
PLOT_Interval = 100
SJZ_Shape_Path = '../../data/raw/gis/shp/shi'

def plotTest():
    jsonPath = '../../data/data_per5min/2017/07/29/merged_data_GMT-8_20170729165000.json'
    xi, yi,zi = Preprocessing(jsonPath=jsonPath, var='PRE')
    plotHumidity(xi, yi, zi, save_dir='./')
...
def Preprocessing(jsonPath, var='TEM', missing='999999'):
    ...
    with open(jsonPath) as data_file:
        ...
        varList = []
        for station in data:
            if not station[var] or station[var] == missing:
...
def plotVisibility(xi, yi, zi, save_dir='.'):
    ...
    # print(zi.shape)
    # 绘图等值线
    PCM = map.pcolor(xx, yy, zi, alpha=1)
    CS = map.contour(xx, yy, zi,\
                alpha=0.8,
                linestyles = 'dashed',
                levels = np.arange(np.min(zi),np.max(zi),1)
